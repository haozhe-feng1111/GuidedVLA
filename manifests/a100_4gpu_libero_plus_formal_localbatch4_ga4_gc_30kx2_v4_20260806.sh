...
# Formal paper-oriented LIBERO training run after the 2026-08-06 server reboot.
#
# 4 x A100 80GB, CUDA devices 0,1,2,3
...
PROJECT_ROOT="${BASE}/repo-libero-author-ga2-20260731"
...
DEPTH_ROOT="${BASE}/models/da3-small-e08cab65"
TOKENIZER_PATH="${BASE}/models/paligemma_tokenizer.model"
ASSETS_ROOT="${BASE}/assets"

RUN_ID="a100_4gpu_libero_plus_formal_localbatch4_ga4_gc_30kx2_v4_20260806"
...
LAUNCHER_PATH="${BASE}/manifests/${RUN_ID}.sh"

AUTHOR_COMMIT="04be059e0d6bd448be5cb45fdbafc775f7eb5e38"
GA2_COMMIT="414653ff6d45cac4dd4703ad9d68d7b08b3a1db8"
...
STAGE1_RUN="pi0_libero_formal_4gpu_localbatch4_ga4_gc_30k_v4_20260806"
STAGE2_RUN="pi0_libero_full_guided_formal_4gpu_localbatch4_ga4_gc_30k_v4_20260806"
...
test "$(git -C "${PROJECT_ROOT}" rev-parse HEAD)" = "${GA2_COMMIT}"
test "$(git -C "${PROJECT_ROOT}" merge-base "${AUTHOR_COMMIT}" "${GA2_COMMIT}")" = "${AUTHOR_COMMIT}"
test "$(git -C "${PROJECT_ROOT}" rev-list --count "${AUTHOR_COMMIT}..${GA2_COMMIT}")" = "1"
...
world_size=${NPROC}
gpu_model_requirement=NVIDIA A100 80GB
...
for index in range(4):
    ...
    assert "A100" in props.name, props.name
    assert props.total_memory >= 79_000_000_000, props.total_memory
...
git -C "${PROJECT_ROOT}" diff "${AUTHOR_COMMIT}..${GA2_COMMIT}" \
    >"${PREFLIGHT_ROOT}/author_release_to_ga2.diff"
