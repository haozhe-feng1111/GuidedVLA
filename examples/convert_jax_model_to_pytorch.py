...
def convert_pi0_checkpoint(
    ...
    # CRITICAL: Create a modified config with float32 dtype for model initialization
    # This ensures weights are loaded without precision loss from bfloat16 conversion
    # The final precision conversion happens after loading weights
    model_config_for_init = copy.copy(model_config)
    object.__setattr__(model_config_for_init, "dtype", "float32")

    # Instantiate model with float32 to avoid precision loss during weight loading
    ...
    # Convert to target precision for saving
    # NOTE: For best accuracy, use float32. The model will be converted to bfloat16
    # at inference time by to_bfloat16_for_selected_params() in policy_config.py
