#!/usr/bin/env python3
"""Compute Stage-1-centered vision CKNNA matrices and paired max-T inference."""
...
POLICIES = ("stage1", "nohead", "da3", "dinov2")
REFERENCES = ("dinov2", "da3")
STREAM_LAYERS = {"paligemma": 19, "siglip": 28}
COMPARISONS = (
    ("nohead", "stage1", "No-head Stage-2 − Stage-1"),
    ("da3", "stage1", "DA3-guided Stage-2 − Stage-1"),
    ("dinov2", "stage1", "DINO-guided Stage-2 − Stage-1"),
)
...
def render_heatmaps(rows: list[dict], stream: str, output_dir: Path) -> None:
    ...
    fig.suptitle(f"GuidedVLA {stream.capitalize()} alignment relative to Stage-1", fontsize=13, fontweight="bold", y=0.985)
    ...
    stem = output_dir / f"guidedvla_{stream}_stage1_centered_reference_layer_sweep"
...
def main() -> None:
    ...
    manifest = {
        "schema_version": "guidedvla-vision-stage1-centered-reference-layer-sweep-v1",
