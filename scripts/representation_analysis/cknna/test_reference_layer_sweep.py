...
import compute_vision_reference_layer_sweep as sweep
...
def test_grid_matches_nested_scalar_cknna() -> None:
    ...
    stream_kernel = torch.bmm(stream, stream.transpose(1, 2))
    reference_kernel = torch.bmm(reference, reference.transpose(1, 2))
...
def test_max_t_simultaneous_intervals_are_not_narrower() -> None:
    rows = []
    rng = np.random.default_rng(7)
    for policy, offset in (("nohead", 0.0), ("da3", 0.01), ("dinov2", 0.03)):
