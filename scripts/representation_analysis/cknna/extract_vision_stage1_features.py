#!/usr/bin/env python3
"""Extract PaliGemma and SigLIP features from the frozen Stage-1-only policy."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-name", choices=("stage1",), required=True)
...
def main() -> None:
    args = parse_args()
    expected_configs = {"stage1": "pi0_libero"}
    ...
    if args.external_encoder_path is not None:
        raise ValueError("Stage-1-only extraction must not load an external encoder")
