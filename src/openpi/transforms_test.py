...
def test_compute_skill_soft_label_from_sequence():
    transform = _transforms.ComputeSkillSoftLabel(num_classes=4)

    data = transform(
        {
            "skill_id": np.array([1, 2, 2]),
        }
    )

    assert data["skill_id"] == 2
    assert np.allclose(data["skill_soft"], np.array([0.0, 1.0 / 3.0, 2.0 / 3.0, 0.0], dtype=np.float32))
...
def test_extract_prompt_from_task():
    ...
    data = transform({"task": "Task from sample"})
    assert data["prompt"] == "Task from sample"
